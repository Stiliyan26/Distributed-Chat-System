...
export function SocketProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    s.on('connect', async () => {
      setConnectionStatus({ type: 'connected', message: 'Connection established', latency: 0 });
      try {
    ...
    s.on('connect_error', (err) => {
      setConnectionStatus({ type: 'error', message: err.message || 'Connection error' });
    });

    s.io.on('reconnect_attempt', () => {
      setConnectionStatus({ type: 'connecting', message: 'Reconnecting to NODE-01...' });
    });
